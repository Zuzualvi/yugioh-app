...
4. **The accurate-text source found.** edisonformat.net's card page (`/card?name=…`, `js/card.js`) shows the modern text only in a collapsed "may not apply to Edison" box, and populates its visible panels from `raw.githubusercontent.com/MikaMikaDE/mikaRulings/main/cards/<passcode>.json` (`PSCT.Edison`, `Rulings.Edison`). Goyo's `PSCT.Edison` is exactly "1 Tuner + 1 or more non-Tuners…". Verified genuinely pre-errata across the hard cases. This is the fix's source of truth (keyed by passcode; distinguishes `Base` modern vs `Edison`).
   ...
   - Fallback for cards mika lacks: **Yugipedia `Card Errata:<name>` pages** give dated, printing-by-printing text history (via the MediaWiki API `action=parse&prop=wikitext`). Used to author the 2 cards mika had no Edison text for (Urgent Tuning, Destiny End Dragoon) by taking the original TCG printing in effect March 2010. Final split: 35 override + 1 no-override (Susa Soldier — errata predates Edison).
   - **PROOFREAD OUTCOME (final):** a proofread of all 35 mika strings against Yugipedia errata history found substantive errors on 8/35 (Necrovalley mis-stated, Cyber Phoenix truncated, Mausoleum garbled, etc.). CEO chose to ship the **verbatim March-2010 printed text** (Yugipedia `lore0` per card) instead of mika's modern-PSCT paraphrase — maximally accurate and citable, at the cost of period templating differing from the modern-text catalog. Detail: `docs/working/2026-07-13-proofread-report.md`.
