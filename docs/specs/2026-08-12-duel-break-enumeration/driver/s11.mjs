...
const { browser, goesFirst: A, goesSecond: B, log, shot } = await setup("s11-full-duel-3");
const seats = [
  { p: A, tag: "seat0" },
  { p: B, tag: "seat1" },
];
...
async function handleQuestion(s, seat, st) {
  ...
  // Prefer an enabled confirm (the selection is already made), else first candidate.
  const cf = st.buttons.find((b) => b.t === "decision-confirm" && !b.d);
  if (cf) {
    log(`[${seat.tag}] confirming: "${cf.x}"`);
    await p.getByTestId("decision-confirm").first().click();
...
async function takeTurn(s, seat, st) {
  ...
  for (let i = 0; i < mzn; i++) {
    ...
    if (await atk.count()) {
      ...
      if ((noopAttacks[key] ?? 0) >= 2) {
        log(`[${seat.tag}] monster[${i}] "${label}" is a NO-OP (clicked twice, board unchanged) — giving up on it`);
        await p.keyboard.press("Escape");
        continue;
      }
      noopAttacks[key] = (noopAttacks[key] ?? 0) + 1;
      attacksThisTurn++;
      const lpBefore = await p.evaluate(() => document.querySelector('[data-testid="opp-lp-plate"]')?.getAttribute("aria-label"));
      log(`[${seat.tag}] ATTACK #${attacksThisTurn} with monster[${i}] label="${label}" oppLpBefore=${lpBefore}`);
      await atk.first().click();
      await p.waitForTimeout(1500);
      const lpAfter = await p.evaluate(() => document.querySelector('[data-testid="opp-lp-plate"]')?.getAttribute("aria-label"));
      log(`[${seat.tag}] after attack: oppLp=${lpAfter} changed=${lpBefore !== lpAfter}`);
      return "attack";
    }
    await p.keyboard.press("Escape");
    await p.waitForTimeout(60);
