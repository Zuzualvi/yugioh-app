--Y-Dragon Head — Edison override
-- Per Edison ruling (R03-B3): only the 6 listed unions provide destroy-instead
-- protection.  Y-Dragon Head is NOT listed → it does NOT protect the equipped
-- monster from any form of destruction (neither battle nor effect).
-- This override omits the EFFECT_DESTROY_SUBSTITUTE (e3) from the standard
-- AddUnionProcedure implementation.
local s,id=GetID()
function s.initial_effect(c)
	local f=aux.FilterBoolFunction(Card.IsCode,62651957)
	--equip (new-style, no oldequip flag)
