/**
 * SelectTributePanel — DecisionPanelProps<"SelectTribute">
 *
 * Tribute material selection. Same multi-select pattern as SelectCardPanel
 * but semantically for tribute targets. Running count (selected / max);
 * Confirm at min; Cancel only when cancelable.
 *
 * Valid candidates pulse (color + outline + scale) — no color-only meaning (§5.3).
...
export default function SelectTributePanel({
  decision: d,
  respond,
  disabled = false,
}: DecisionPanelProps<"SelectTribute">) {
  ...
  function handleConfirm() {
    ...
    respond({ kind: "SelectTribute", indices: selectedIndices });
  }
  ...
  return (
    <div style={PANEL}>
      <style>{`
        .select-tribute-pulse {
          animation: select-tribute-pulse-anim 1.2s ease-in-out infinite;
        }
        @keyframes select-tribute-pulse-anim {
        ...
        @media (prefers-reduced-motion: reduce) {
          .select-tribute-pulse { animation: none; }
        }
      `}</style>
      ...
      {/* Tribute candidate list */}
      <div style={CARD_LIST} role="group" aria-label="Tribute candidates">
        {d.cards.map((card, i) => {
          ...
          return (
            <button
              ...
              className={isValid ? "select-tribute-pulse" : undefined}
              style={{
