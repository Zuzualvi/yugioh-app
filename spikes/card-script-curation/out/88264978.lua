...
--EDISON FIX: changed SetCountLimit(1,id,EFFECT_COUNT_CODE_OATH) → SetCountLimit(1) (summon procedure)
--EDISON FIX: changed SetCountLimit(1,{id,1}) → SetCountLimit(1) (ignition effect)
--Result: per-copy limits only; no once-per-NAME restriction. Multiple copies can each use their own effects.
local s,id=GetID()
